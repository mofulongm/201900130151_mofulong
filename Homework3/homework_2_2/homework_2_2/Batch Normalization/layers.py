...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_ = (x - m) / np.sqrt(sigma2 + eps)
        out = gamma * x_ + beta
        sigma_2 = 1. / np.sqrt(sigma2 + eps)
        cache = (x, x_, gamma, m, sigma_2)
...
def batchnorm_backward(dout, cache):
    ...
    dvar = np.sum(-0.5 * sigma_2 ** 3 * (x - m) * gamma * dout, axis=0)
    dm = np.sum(-1 * sigma_2 * gamma * dout, axis=0)
    dx = gamma * dout * sigma_2 + (2 / N) * (x - m) * dvar + (1 / N) * dm
    pass
